...
class Sidebar::MenuComponent < ViewComponent::Base
  MenuItem = Struct.new(:icon, :name, :path, :method, keyword_init: true)
  ...
  def settings_items
    [
      ...
      menu_item(icon: "fas fa-sign-out-alt",  name: t("users.menu.sign_out"), path: destroy_user_session_path, method: :delete)
    ]
  end

  def menu_item(icon:, name:, path:, method: nil)
    MenuItem.new(icon: icon, name: name, path: path.presence, method: method)
  end
end
